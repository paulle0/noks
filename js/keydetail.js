...
import {
  addKeyEntry, removeKeyEntry,
  publishMasterPublicKeyring, publishMasterPrivateKeyring,
  publishSubkeyKeyring, publishSubkeyPrivateKeyring,
} from "../keyring.js";
...
async function renderExistingKey(root, key) {
  ...
  root.innerHTML = `
    ...
    <div class="detail-section">
      <h4>Publish</h4>
      <p class="card-subtitle" style="margin-bottom:var(--space-3);">
        Publishes both kind 17991 (public) and kind 17992 (private, encrypted)
        keyring events to your home relays.
      </p>
      <div class="share-actions">
        <button class="btn-secondary" id="pubMaster">Publish masterkey keyring</button>
        ${key.seckey ? `<button class="btn-secondary" id="pubSub">Publish subkey keyring</button>` : ""}
      </div>
    </div>`;
  ...
  root.querySelector("#pubMaster").addEventListener("click", () => onPublishMaster());
  const pubSub = root.querySelector("#pubSub");
  if (pubSub) pubSub.addEventListener("click", () => onPublishSubkey(key));
}

async function onPublishMaster() {
  toast("Publishing masterkey keyring (17991 + 17992)…");
  ...
  summarize(resPub, "Master 17991");
  summarize(resPriv, "Master 17992");
}

async function onPublishSubkey(key) {
  toast("Publishing subkey keyring (17991 + 17992)…");
  const [resPub, resPriv] = await Promise.all([
    publishSubkeyKeyring(key).catch((e) => [{ ok: false, error: e.message }]),
    publishSubkeyPrivateKeyring(key).catch((e) => [{ ok: false, error: e.message }]),
  ]);
  summarize(resPub, "Subkey 17991");
  summarize(resPriv, "Subkey 17992");
}
...
async function onDelete(key) {
  const ok = await modal({
    ...
    body: `<p>Removes <strong>${escapeHtml(key.name || shortHex(key.pubkey))}</strong> from your local keyring. Publishing a fresh masterkey keyring afterwards will remove it from your remote keyring as well.</p>`,
...
function summarize(results, label) {
  ...
  if (ok === total) toast(`${label} → ${ok}/${total} relays`, "success");
  else if (ok > 0) toast(`${label} → ${ok}/${total} relays`, "info");
